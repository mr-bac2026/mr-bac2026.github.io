...
top |>
  ...
  gt() |>
  fmt_percent(columns = `Taux d'admission`, decimals = 1,
              dec_mark = ",", sep_mark = " ") |>
  ...
  tab_header(title = md("**Les douze établissements les mieux classés**"),
             subtitle = paste("Parmi ceux ayant présenté au moins 30",
                              "candidats (session normale 2026)")) |>
  tab_source_note(md(SRC_NOTE)) |>
...
ggplot() +
  ...
  labs(title = "Les établissements qui sortent vraiment du lot",
       subtitle = paste("Chaque point est un établissement.",
                        "Bandes : intervalles attendus à 95 % et 99 %."),
       x = "Nombre de candidats (échelle logarithmique)",
       y = "Taux d'admission",
       caption = SRC) +
  theme_bac()
```
...
solides |>
  ...
  gt() |>
  fmt_percent(columns = c(Taux, `IC 95 % (bas)`), decimals = 1,
              dec_mark = ",", sep_mark = " ") |>
  data_color(columns = Taux,
             fn = scales::col_numeric(BAC_SEQ_CHAUD, domain = NULL)) |>
  cols_align("left", columns = c(Établissement, Wilaya)) |>
  tab_header(title = md("**Les établissements les plus solides**"),
             subtitle = paste("Au moins 50 candidats et borne basse",
                              "de l'IC 95 % au-dessus de la moyenne",
                              "nationale")) |>
  tab_source_note(md(SRC_NOTE)) |>
  tab_options(table.font.size = px(13), table.width = pct(100),
              heading.title.font.size = px(16))
