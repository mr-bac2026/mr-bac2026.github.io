...
ggplot(rep, aes(x = n, y = fct_rev(decision), fill = decision)) +
  geom_col(width = 0.68) +
  geom_text(aes(label = paste0(format(n, big.mark = " "), "  ·  ", pct_fr(part))),
            hjust = -0.05, family = BAC_FONT_BODY, size = 3.1, colour = BAC_COL$encre) +
  scale_fill_manual(values = BAC_PAL_DECISION, guide = "none") +
  scale_x_continuous(limits = c(0, max(rep$n) * 1.22), expand = expansion(c(0, 0))) +
  labs(title = "Un admis pour sept ajournés",
       subtitle = glue("Répartition des {format(n_tot, big.mark=' ')} candidats de la session normale selon la décision finale."),
...
#| fig-width: 8
#| fig-height: 3.8
#| fig-alt: "Entonnoir des candidats : inscrits, présents, au-dessus de 8, admis."
etapes <- tibble::tibble(
  etape = factor(c("Inscrits", "Présents à l'épreuve", "Moyenne ≥ 8", "Admis (≥ 10)"),
                 levels = c("Inscrits", "Présents à l'épreuve", "Moyenne ≥ 8", "Admis (≥ 10)")),
  n = c(n_tot,
        sum(d$present == 1),
...
ggplot(etapes, aes(x = etape, y = n)) +
  ...
  geom_text(aes(label = paste0(format(n, big.mark = " "), "\n", pct_fr(part, 1))),
            vjust = -0.25, family = BAC_FONT_BODY, size = 3.1, colour = BAC_COL$encre,
            lineheight = 0.95) +
...
ggplot(pres, aes(x = moyenne)) +
  geom_histogram(aes(y = after_stat(density)), binwidth = 0.5, boundary = 0,
                 fill = BAC_COL$papier_alt, colour = BAC_COL$papier, linewidth = 0.15) +
  ...
  annotate("text", x = med, y = Inf, label = paste0("médiane ", num_fr(med, 0.1)),
           hjust = -0.08, vjust = 2, family = BAC_FONT_BODY, size = 3, colour = BAC_COL$vert) +
  scale_x_continuous(breaks = seq(0, 18, 2), limits = c(0, 18)) +
  labs(title = "Une distribution qui penche à gauche",
       subtitle = "Densité des moyennes des candidats présents. Les tirets marquent les seuils de 8 et 10.",
...
ggplot(men, aes(x = mention, y = n)) +
  ...
  geom_text(aes(label = paste0(format(n, big.mark = " "), " · ", pct_fr(part))),
            vjust = -0.3, family = BAC_FONT_BODY, size = 3, colour = BAC_COL$encre) +
  scale_y_continuous(labels = label_number(big.mark = " "),
