...
ggplot(carte_j, aes(long, lat, group = group)) +
  ...
  coord_quickmap() +
  labs(
    title = "La plupart des candidats sont au Sud et à Nouakchott",
    subtitle = paste("Nombre de candidats à la session normale 2026,",
...
ggplot(par_pop, aes(x = pop, y = fct_reorder(shapeName, pop))) +
  ...
  scale_x_continuous(limits = c(0, max(par_pop$pop) * 1.15),
...
ggplot(par_pop, aes(x = pm, y = fct_reorder(shapeName, pm))) +
  ...
  annotate("text", x = nat_pm, y = 0.7,
           label = paste0("moyenne : ", num_fr(nat_pm, 0.1)),
           hjust = -0.05, family = BAC_FONT_BODY, size = 2.7,
           colour = BAC_COL$encre_douce) +
  scale_fill_gradientn(colours = BAC_SEQ_CHAUD, guide = "none") +
  scale_x_continuous(limits = c(0, max(par_pop$pm) * 1.12),
                     expand = expansion(c(0, 0))) +
  labs(title = "Du simple au sextuple selon la région",
       subtitle = paste("Nombre de candidats au baccalauréat 2026",
...
ggplot(carte_j, aes(long, lat, group = group)) +
  ...
  labs(title = "Le taux d'admission ne suit pas le nombre de candidats",
       subtitle = paste("Part des candidats admis à la session normale 2026,",
                        "par région."),
       caption = paste(SRC, "Fonds : geoBoundaries (ADM1).", sep = "\n")) +
  theme_bac_carte()
```
...
ggplot(par_wilaya, aes(x = taux, y = fct_reorder(nom, taux))) +
  ...
  labs(title = "Le taux d'admission varie fortement selon la wilaya",
       subtitle = paste("Taux d'admission et intervalle de confiance à 95 %",
                        "(méthode de Wilson)."),
...
ggplot(par_pole, aes(x = taux, y = pole)) +
  ...
  geom_text(aes(label = paste0(pct_fr(taux), "  (",
                               format(n, big.mark = " "), " candidats)")),
            hjust = -0.12, family = BAC_FONT_BODY, size = 3.1,
            colour = BAC_COL$encre) +
  scale_x_continuous(labels = label_percent(accuracy = 1),
                     limits = c(0, max(par_pole$taux) * 1.5)) +
  labs(title = "Cinq points d'écart entre Nouakchott et l'intérieur",
       subtitle = paste("Taux d'admission comparé de la capitale et de",
